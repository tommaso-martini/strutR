...
Between two consecutive fronts \(x_{i-1}\) and \(x_i\) the profile is **piecewise constant** at \(\theta_i\) (rectangular layers). The residual and saturated water contents are \(\theta_{\mathrm{r}}\) and \(\theta_{\mathrm{s}}\), respectively.

We assume a Brooks–Corey conductivity model
$$
K(\theta) \;=\; K_s \left(\frac{\theta-\theta_{\mathrm{r}}}{\theta_{\mathrm{s}}-\theta_{\mathrm{r}}}\right)^{1/b},
$$
with \(K_s\) [$m\,\mathrm{d}^{-1}$] and shape parameter \(b>0\).

## Notation

- \(L\) [m]: profile depth.  
- \(\delta\) [m]: depth at which the wetting model starts (often \(\delta=0\)).  
- \(f(t)\) [$m\,\mathrm{d}^{-1}$]: infiltration supply at the soil surface.  
- \(\theta_{\star}\) (theta\_star): **stress onset** threshold.  
- \(\theta_{\mathrm{wp}}\) (theta\_wp): **wilting** threshold for transpiration.  
- \(Z_{\text{root}}\) [m]: active root-zone thickness from the surface; \(Z_{\text{evap}}\) [m]: active thickness for bare-soil evaporation.  
- \(e_p\) [$m\,\mathrm{d}^{-1}$]: potential evaporation (reference ET) per day; \(C_v\in[0,1]\) fraction allocated to transpiration on that day.
...
# Evapotranspiration with \(\theta_{\star}\) and \(\theta_{\mathrm{wp}}\)
...
$$
\bar{\theta}_{\text{root}} \;=\;
\frac{1}{Z_{\text{root}}}\int_{\delta}^{\delta+Z_{\text{root}}}\theta(z)\,\,\mathrm{d}z
\;\approx\;
...
## Transpiration \(E_t\) (Eqs. 10–12, with \(\theta_{\star}\), \(\theta_{\mathrm{wp}}\))

1. **Potential (unstressed) phase** up to \(\theta_{\star}\).  
   Water needed to raise/reduce the mean to \(\theta_{\star}\):
   $$
   S_{\star} \;=\; \max\!\left(0,\ \bigl(\bar{\theta}_{\text{root}}-\theta_{\star}\bigr)\,Z_{\text{root}}\right),
   \qquad
   t_{\star} \;=\; \frac{S_{\star}}{D_t}.
   $$
   If \(t_{\star}\ge \Delta t\) then \(E_t = D_t\) (all potential).

2. **Stressed phase** between \(\theta_{\star}\) and \(\theta_{\mathrm{wp}}\).  
   The available storage above \(\theta_{\mathrm{wp}}\) in the root-zone is
   $$
   S_{\mathrm{avail},\wp} \;=\; \int_{\delta}^{\delta+Z_{\text{root}}}\max\big(\theta(z)-\theta_{\mathrm{wp}},\,0\big)\,\,\mathrm{d}z.
   $$
   ...
   S_{\mathrm{cap}} = (\theta_{\star}-\theta_{\mathrm{wp}})\,Z_{\text{root}}.
   $$
   The total target is \(E_t^\text{target} = \min\{\,S_{\mathrm{avail},\wp},\ S_\star + E_{t,\mathrm{stress}}\,\}\).

3. **Application rule (volume removal)**.  
   Remove \(E_t^\text{target}\) **top–down** from the root-zone, reducing \(\theta\) of the shallowest layers first while enforcing \(\theta\ge \max(\theta_{\mathrm{wp}},\theta_{\mathrm{r}})\). If a layer’s storage
   \((x_i-x_{i-1})(\theta_i-\theta_{i-1})\) is exhausted, drop that front (merge).

## Bare-soil evaporation \(E_{bs}\) (Eq. 9)

Define the **evaporative layer** \([\,\delta,\ \delta+Z_{\text{evap}}\,]\) (typically \(Z_{\text{evap}}\!\sim\!0.05\)–$0.1\ \mathrm{m}$). Its available storage above \(\theta_{\mathrm{r}}\) is
$$
S_{\mathrm{evap}} \;=\; \int_{\delta}^{\delta+Z_{\text{evap}}}\max\big(\theta(z)-\theta_{\mathrm{r}},\,0\big)\,\,\mathrm{d}z.
$$
The daily target is
$$
...
$$
applied top–down within \([\,\delta,\delta+Z_{\text{evap}}\,]\) while enforcing \(\theta\ge\theta_{\mathrm{r}}\).
...
\Big(\max(\theta_{i,j},\theta_{\mathrm{wp}}) - \max(\theta_{i-1,j},\theta_{\mathrm{wp}})\Big)}
{\displaystyle \sum_{m=1}^{J}\sum_{i\in \mathcal I_m}
\Big(\max(\theta_{i,m},\theta_{\mathrm{wp}}) - \max(\theta_{i-1,m},\theta_{\mathrm{wp}})\Big)},
\qquad \sum_j f_j = 1,
$$
...
- Clamp \(\theta\in[\theta_{\mathrm{r}},\theta_{\mathrm{s}}]\).
...
The following example demonstrates I–R–D with daily ET using \(\theta_{\star}\) and \(\theta_{\mathrm{wp}}\). Units are meters and days.
